...
上述代码中，`p`的store是`memory_order_release`的，它的load是`memory_order_consume`的，循环保证了load能够读取到store的指针，因此当load能够读取到store的指针时，`p`的store dependency-ordered-before 它的load。所以`p`的store happens-before 它的load。由于通过变量`x`，`p`的load carries-a-dependency-to `x`的load，因此对于`x`的两个assert不会触发。但`a`的load读取到的值是没有保证的。
