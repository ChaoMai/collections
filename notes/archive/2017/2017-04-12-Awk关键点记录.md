...
* ARGC：命令行参数数目。
* ARGV：命令行参数，索引从0起。
* ARGIND：当前正在处理的的文件ARGV的索引，对于一个正在处理的文件，`FILENAME == ARGV[ARGIND]`总为真。
...
```awk
cat imei_not_hit.002.txt | awk '{
    command="echo -n "$1" | md5sum" ;
    command | getline l;
    close(command);
    sub(/[[:blank:]].*/, "", l);
    print l, 1
...
```awk
cat file | awk '{print > $1_file_suffix}'
```
